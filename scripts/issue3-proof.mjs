 ...
 *
 * Structural guarantees (bounds, font floor, no unsupported SVG features)
 * live in test/density.test.ts where CI enforces them over a wider matrix;
 * this script fails hard if sharp cannot rasterize any face.
 */
...
if (outDir === undefined) {
	console.error("Usage: node --import tsx scripts/issue3-proof.mjs <outDir>");
	process.exit(1);
}
...
async function sheet(px, file) {
	...
	for (const section of sections) {
		...
		headers.push(`<text x="8" y="${y + 14}" font-family="Segoe UI, Arial, sans-serif" font-size="13" font-weight="700" fill="#9aa3b2">${section.toUpperCase()}</text>`);
		y += 22;
		for (let i = 0; i < group.length; i++) {
			...
			headers.push(`<text x="${left}" y="${top + px + 11}" font-family="Segoe UI, Arial, sans-serif" font-size="9" fill="#6b7280">${id}</text>`);
		}
...
await sheet(144, "issue3-proof-144.png");
await sheet(72, "issue3-proof-72.png");
await deckSim("issue3-deck-sim.png");
console.log(`ISSUE3 PROOF: ${faces.length} faces rasterized to ${outDir}`);
