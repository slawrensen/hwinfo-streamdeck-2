...
- **Anchors never move.** The label, value, unit and sparkline sit at the same fixed positions on every key, with or without a sparkline, across every theme. That constancy is what makes a mixed wall of temperatures, clocks and fan speeds read as one instrument instead of a collage.
