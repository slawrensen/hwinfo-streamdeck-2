...
![A contact sheet of the same sensor rendered in all 7 themes side by side, showing the rang]({{ '/assets/img/themes-contact-sheet.png' | relative_url }})
...
Set the deck-wide theme under **Advanced → Deck theme** in any key's or dial's settings (it's a global setting — there's one value for the whole plugin).
...
![The Theme gallery in the property inspector: the dashed "Deck default" chip with its link ]({{ '/assets/img/settings-panel.png' | relative_url }})
...
Turn type accents **off** (*Advanced → Type accents → Off*) to use the theme's accent color everywhere instead.
